  ...
  methods: {
    drawScoreChart() {
      securityAssess.getOnlineNIPScore().then((resp) => {
        if(resp.data.onlineNIPScore===undefined){
          this.nipTerminalCompreScoreChartOption.series[0].data[0].value =0.0
        }else{
          this.nipTerminalCompreScoreChartOption.series[0].data[0].value = resp.data.onlineNIPScore/100;
        }
